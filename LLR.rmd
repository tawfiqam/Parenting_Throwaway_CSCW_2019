...
lines <- readLines("CPS_Child_Abuse_throw.txt", encoding = "UTF-8")
...
targetCorpus <- tm_map(targetCorpus, removePuctuation)

targetDTM <- DocumentTermMatrix(targetCorpus)
termCountsTarget <- col_sums(targetDTM)

lines <- readLines("CPS_Child_Abuse_pseud.txt", encoding = "UTF-8")
...
comparisonCorpus <- tm_map(comparisonCorpus, removePunctuation))
...
sort(logLikelihood, decreasing=FALSE)[1:50]